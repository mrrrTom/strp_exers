...
string collect_cur_string(istream& is) {
	char cur_symb;
	string result;
	while(is.get(cur_symb) && cur_symb != ' ' && cur_symb != '(') {
		result += cur_symb;
	}

	if (cur_symb == '(') is.putback('(');
	return result;
}
class Token_stream {
	public:
			...
			switch (ch) {
				...
				default:
					if (isalpha(ch)) {
						ip -> putback(ch);
						istream& tmp {cin};
						ct.string_value = collect_cur_string(tmp);
						ct.kind = Kind::name;
						return ct;
					}
...
double prim (bool get) {
	...
	switch (ts.current().kind) {
		...
		case Kind::name:
			{
				string name_val = ts.current().string_value;
				if (funcs.find(name_val) != funcs.end()) {
					ts.get();
