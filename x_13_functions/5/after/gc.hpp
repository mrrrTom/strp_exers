...
header* NEXT_HEADER(header* x) { return (header*)((size_t)(x+1) + x->size); }

/* flags */
constexpr int FL_ALLOC = 0x1;
constexpr int FL_MARK = 0x2;
void FL_SET(void* x, int f) { ((header *)x)->flags |= f; }
void FL_UNSET(void* x, int f) { ((header*)x)->flags &= ~(f); }
void FL_TEST(void* x, int f) { ((header *)x)->flags & f; }
